    ...
    return (<>
        ...
        <div className="Columns">
            <div>
                ...
                <input placeholder={"type something"} onChange={({target: {value}}) => setText(value)}
                       value={text}/>
            ...
            <div>
                ...
                <input placeholder={"type something"} onChange={({target: {value}}) => setTextWithoutBatching(value)}
                       value={textWithoutBatching}/>
                <button onClick={createTodoWithoutBatching}>Add todo</button>
                <ul>
                    {todosWithoutBatching.map((todo, key) => <li key={key}>{todo}</li>)}
                </ul>
            </div>
        </div>
    </>);
